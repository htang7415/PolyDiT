...
DISCRETE_DIFFUSION_FILES = [
    ...
    Path("Bi_Diffusion_SMILES_BPE/src/model/diffusion.py"),
...
def test_discrete_diffusion_has_clean_t0_gate_and_zero_loss_guard():
    for file_path in DISCRETE_DIFFUSION_FILES:
        ...
        assert "valid_count.clamp_min(1.0)" in source, f"Missing empty-mask denominator guard in {file_path}"
        assert "zero_loss = logits.sum() * 0.0" in source, f"Missing connected zero-loss value in {file_path}"
        assert "torch.where(valid_count > 0, safe_loss, zero_loss)" in source, f"Missing compile-safe zero-loss guard in {file_path}"
