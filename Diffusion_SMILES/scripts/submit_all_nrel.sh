...
# Usage: bash scripts/submit_all_nrel.sh <model_size> [property] [targets] [epsilon]
#        [num_samples] [num_candidates] [polymer_class] [class_target] [class_epsilon] [partition] [qos]
...
sbatch "${SBATCH_ARGS[@]}" --job-name "$JOB2" --output "logs/%x_%j.out" --error "logs/%x_%j.err" --dependency=afterok:$jid scripts/submit_steps2_6_nrel.sh   "$MODEL_SIZE" "$PROPERTY" "$TARGETS" "$EPSILON" "$NUM_SAMPLES"   "$NUM_CANDIDATES" "$POLYMER_CLASS" "$CLASS_TARGET" "$CLASS_EPSILON"
